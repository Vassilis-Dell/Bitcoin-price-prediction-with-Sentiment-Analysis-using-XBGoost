...
if 'Close' in df.columns:
    ...
    df['MACD_Signal'] = ta.trend.macd_signal(df['Close'].squeeze())  # Add MACD Signal
    ...
    df['Price_Change_Abs'] = df['Close'].diff().abs()  # Absolute price change
    df['Volume_Normalized'] = (df['Volume'] - df['Volume'].mean()) / df['Volume'].std()  # Normalize volume
    df['Volume_Price_Interaction'] = df['Volume_Normalized'] * df['Price_Change_Abs']  # Interaction term
...
# 1.1 Correlation Analysis (Technical Indicators and Price)
correlation_matrix_technical = df[['Close', 'SMA_5', 'SMA_20', 'SMA_50', 'RSI', 'MACD', 'MACD_Signal', 'Volatility', 'BB_upper', 'BB_lower', 'Open', 'High', 'Low', 'Volume']].corr()
print("\nCorrelation Matrix of Technical Indicators and Price:")
print(correlation_matrix_technical['Close'].sort_values(ascending=False))

plt.figure(figsize=(12, 10))
sns.heatmap(correlation_matrix_technical, annot=True, cmap='coolwarm', fmt=".2f")
plt.title('Correlation Matrix of Technical Indicators and Price')
plt.savefig('correlation_matrix_technical.png')
plt.close()

# 1.2 Correlation Analysis (Sentiment and Price/Technical Indicators)
correlation_matrix_sentiment = df[['Close', 'SMA_5', 'SMA_20', 'SMA_50', 'RSI', 'MACD', 'MACD_Signal', 'Volatility', 'BB_upper', 'BB_lower', 'Volume', 'Sentiment']].corr()
print("\nCorrelation Matrix of Sentiment with Price and Technical Indicators:")
...
plt.savefig('correlation_matrix_sentiment.png')
plt.close()

# 1.3 Correlation among all XGBoost Input Features
features_for_correlation = ['Open', 'High', 'Low', 'Close', 'Volume', 'SMA_5', 'SMA_20', 'SMA_50', 'RSI', 'MACD', 'MACD_Signal', 'Volatility', 'BB_upper', 'BB_lower',
...
print("\nCorrelation Matrix of All XGBoost Input Features:")
print(correlation_matrix_all)

plt.figure(figsize=(16, 14))
sns.heatmap(correlation_matrix_all, annot=True, cmap='magma', fmt=".2f")
plt.title('Heatmap of Correlation Matrix of All XGBoost Input Features')
...
# Volume-Price Analysis: Scatter Plot
plt.figure(figsize=(10, 6))
plt.scatter(df['Volume'], df['Price_Change_Abs'], alpha=0.5)
plt.xlabel('Trading Volume (USD)')
plt.ylabel('Absolute Price Change (USD)')
plt.title('Volume vs. Absolute Price Change')
plt.savefig('volume_price_relationship.png')
plt.close()
...
# 7. Real-Time Data (180 days before the last sentiment date)
def get_realtime_data():
    try:
        # Define the end date as the last sentiment date
        ...
        # Start date is 180 days before the last sentiment date
        ...
        # Flatten column names (yfinance sometimes returns MultiIndex)
        ...
        last_price_data['MACD_Signal'] = ta.trend.macd_signal(pd.Series(last_price_data['Close'].squeeze()))  # Add MACD Signal
        ...
        # Merge with sentiment data for this period
        ...
        # Forward-fill any remaining NaNs in the technical indicators
        return last_price_data.ffill()
    except Exception as e:
        print(f"Error getting real-time data: {e}")
        return None

last_days_data = get_realtime_data()

if last_days_data is not None and not last_days_data.empty:
    pred_no_sentiment = predict_price(last_days_data, best_model_no_sentiment, scaler_no_sentiment, include_sentiment=False)
    pred_with_sentiment = predict_price(last_days_data, best_model_with_sentiment, scaler, include_sentiment=True)
    print(f"Predicted Price (No Sentiment): ${pred_no_sentiment[-1]:.2f}")
    print(f"Predicted Price (With Sentiment): ${pred_with_sentiment[-1]:.2f}")
else:
    print("Failed to fetch real-time data for the specified period.")

# 8. Plotting Actual vs. Predicted (180 days)
if last_days_data is not None and not last_days_data.empty:
    actual_prices_180d = last_days_data['Close']
    X_180d = last_days_data[-len(actual_prices_180d):]
    ...
    plt.plot(actual_prices_180d.index, actual_prices_180d.values, label="Actual Price", color="blue")
    plt.plot(X_180d.index, pred_180d_no, label="Predicted (No Sentiment)", color="red")
    plt.plot(X_180d.index, pred_180d_with, label="Predicted (With Sentiment)", color="green")
    plt.title("Actual vs. Predicted Bitcoin Price (Last 180 Days)")
    plt.xlabel("Date")
    plt.ylabel("Price (USD)")
    plt.legend()
    plt.grid(True)
    plt.savefig("actual_vs_predicted_sentiment_comparison.png")
    plt.close()

# 9. Future Price Prediction (90 days starting from the last sentiment date)
# Use the last 180 days of historical data (up to the last sentiment date) for rolling predictions
last_180_days = df[-180:].copy()
if last_180_days is not None and not last_180_days.empty:
    ...
    for i in range(future_days):
        ...
        rolling_window['MACD_Signal'] = ta.trend.macd_signal(pd.Series(rolling_window['Close'].squeeze()))  # Add MACD Signal
        ...
        if next_price_with is not None and next_price_no is not None:
            ...
            chosen_volatility = (1 - weight) * volatility_forecast[i] + weight * volatility_forecast_3y[i]
            ...
            predicted_sentiment = last_sentiment + np.random.uniform(-0.1, 0.1)  # Small random walk
            predicted_sentiment = np.clip(predicted_sentiment, -1, 1)  # Keep between -1 and 1
    ...
    predictions_df_no = pd.DataFrame({
        ...
        'High': future_highs_no,'Low': future_lows_no,
        'Volume': future_volumes
    })
    ...
    plt.plot(last_180_days['Close'].index, last_180_days['Close'].values, label="Actual Price (Last 180 Days)", color="blue")
    plt.plot(future_dates[:len(future_prices_with_sentiment)], future_prices_with_sentiment, label="Predicted (With Sentiment)", color="green", marker='o')
    plt.plot(future_dates[:len(future_prices_no_sentiment)], future_prices_no_sentiment, label="Predicted (No Sentiment)", color="red", marker='o')
    plt.fill_between(future_dates[:len(future_prices_with_sentiment)], future_lows_with, future_highs_with, color='green', alpha=0.2, label='High-Low Range (With Sentiment)')
    plt.fill_between(future_dates[:len(future_prices_no_sentiment)], future_lows_no, future_highs_no, color='red', alpha=0.2, label='High-Low Range (No Sentiment)')
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("future_prediction_90d_garch_with_and_without_sentiment.png")
    plt.close()

    # Print predictions
    print("\nFuture Bitcoin Price Predictions (All 90 Days):")
    print("{:<12} {:<20} {:<20} {:<20} {:<20} {:<15} {:<15}".format(
    ...
    for date, close_w, high_w, low_w, close_n, volume, sentiment in zip(
        ...
        print("{:<12} ${:<19.2f} ${:<19.2f} ${:<19.2f} ${:<19.2f} {:<15.0f} {:<15.2f}".format(
